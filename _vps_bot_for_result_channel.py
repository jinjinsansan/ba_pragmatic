...
V4_PATTERNS = frozenset({
    "sansan|telecho|P", "bline|telecho|B", "telecho|niconico|P",
...
def _send_telegram_result(text: str) -> bool:
    ...
    chat_id = os.getenv("DUAL_LINE_RESULT_CHAT_ID", "").strip()
    if not chat_id:
        return False
    token = (
        os.getenv("DUAL_LINE_BOT_TOKEN", "").strip()
    ...
    try:
        ...
        if not response.ok:
            logger.warning(
                f"[TELEGRAM-RESULT] send failed status={response.status_code} "
                f"response={response.text[:160]!r}"
            )
            return False
        return True
    except Exception as e:
        logger.warning(f"[TELEGRAM-RESULT] send failed: {e}")
        return False
...
class DualLinePragmaticBot(cp.Collector):
    ...
    def _follow_sim_stats_line(st):
        ...
        return (
            "── 追従は得か?損か?(対象パターン限定) ──\n"
            "追従のみ: %dW/%dL/%dT (%.1f%%) pnl %+.1f ←ここが+なら得\n"
            "NOW単独: %dW/%dL/%dT (%.1f%%) pnl %+.1f\n"
            "追従込み: %dW/%dL (%.1f%%) pnl %+.1f\n"
            "最高連勝: %d | chains: %d" % (
    ...
    def _resolve_prediction(
        ...
        _v3_res_text = (
            ...
            f"Pattern: {_fmt_pattern(pkey)}\n"
            f"Pred: {side} → Got: {outcome}\n"
            f"W/L/T: {self.wins}/{self.losses}/{self.ties} ({_v3_wr:.1f}%)\n"
