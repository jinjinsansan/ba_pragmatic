...
# analytics.sqlite3 (shoe/hands + crowd_events)
from analytics_db import init_db as init_analytics_db
from analytics_db import save_crowd_event
...
# Ensure analytics DB schema exists (safe to call multiple times)
try:
    init_analytics_db()
except Exception as e:
    logger.warning(f"analytics DB init failed (continuing): {e}")
...
class ResultRequest(BaseModel):
    ...
    # --- optional crowd features (table-enter WS only) ---
    captured_at: str = ""
    table_id: str = ""
    table_name: str = ""
    game_id: str = ""
...
async def submit_result(
    ...
    with _sessions_lock:
        ...
        # Crowd stats persistence (best-effort; never blocks result processing)
        try:
            if req.captured_at and req.crowd:
                save_crowd_event(
                    user_id=user_id,
                    event_type="bettingStats_at_result",
                    captured_at=req.captured_at,
                    table_id=req.table_id or None,
                    table_name=req.table_name or None,
                    game_id=req.game_id or None,
                    bet_side=req.side or None,
                    bet_amount=float(req.bet_amount) if req.bet_amount is not None else None,
                    result=req.result or None,
                    crowd=req.crowd or {},
                )
        except Exception:
            pass
