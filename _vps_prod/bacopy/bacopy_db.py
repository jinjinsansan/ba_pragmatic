...
_LAST_PENDING_SWEEP = [0.0]


def expire_stale_pending(max_age_sec: float) -> int:
    """status='pending' のまま max_age_sec 秒を超えた decision を error へ強制遷移する。
    受け子は >20s の decision を stale として捨てるため、放置すると pending が無限堆積し、
    /wait・/pending の `ORDER BY received_at DESC LIMIT` で新鮮な decision を押し出して
    しまう(v4 等が解決されず溜まると v3 が LIMIT 外に消える)。これを根絶する自動 expire。
    戻り値は expire した件数。"""
    conn = sqlite3.connect(_db_path())
    try:
        cur = conn.cursor()
        cur.execute("SELECT decision_id, received_at FROM decisions WHERE status='pending'")
        rows = cur.fetchall()
        now = datetime.now(timezone.utc)
        expired = 0
        for did, at in rows:
            try:
                ts = datetime.fromisoformat(str(at or "").replace("Z", "+00:00"))
            ...
            if age > max_age_sec:
                payload = json.dumps(
                    {"error": "auto_expired_stale_pending", "age_sec": round(age, 1)},
                    ensure_ascii=False,
                )
                cur.execute(
                    "UPDATE decisions SET status='error', result_json=? "
...
def _maybe_expire_pending() -> None:
    """get_by_status('pending') から呼ぶ throttle 付き sweep(最大 5 秒に 1 回)。
    BACOPY_PENDING_MAX_AGE_SEC=0 で無効化可能。"""
    try:
        max_age = float(os.getenv("BACOPY_PENDING_MAX_AGE_SEC", "60") or 60)
        if max_age <= 0:
            return
        now = time.time()
        if now - _LAST_PENDING_SWEEP[0] < 5.0:
            return
        _LAST_PENDING_SWEEP[0] = now
        expire_stale_pending(max_age)
    except Exception:
        pass
