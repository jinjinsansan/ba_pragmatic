...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # noqa: N802
        ...
        if u.path == "/api/winrate-history":
            # GUI 勝率FX風チャート: cron(/opt/laplace2/winrate_history.py)が10分毎に
            # 蓄積する永続バケット(pattern別 ymd/h/n/w・midnightリセットなし)を返す。
            try:
                _wh = Path("/opt/bacopy/winrate_history.json")
                _wh_data = json.loads(_wh.read_text(encoding="utf-8")) if _wh.exists() else {"ok": False, "error": "not_ready"}
            except Exception as _whe:
                _wh_data = {"ok": False, "error": str(_whe)}
            return _send_json(self, 200, _wh_data)
        if u.path == "/api/winrate-trend":
            # GUI 勝率レンジチャート: cron(/opt/laplace2/winrate_trend.py)が10分毎に
            # 蓄積するチャンネル一致の累計勝率時系列(pattern=self.wins/self._v4, follow=follow_sim)。
            try:
                _wt = Path("/opt/bacopy/winrate_trend.json")
                _wt_data = json.loads(_wt.read_text(encoding="utf-8")) if _wt.exists() else {"ok": False, "error": "not_ready"}
            except Exception as _wte:
                _wt_data = {"ok": False, "error": str(_wte)}
            return _send_json(self, 200, _wt_data)
        if u.path == "/api/hourly-stats":
