...
# 冷間起動でロビーへ直リンクすると third party session が張れない既知不具合の回避。
# 先に casino/home を踏んでセッションを温めてからロビーへ入る (2026-08-03)。
HOME_URL = os.getenv("BACOPY_LOBBY_HOME_URL", "https://stake.com/ja/casino/home")
...
class Collector:
    ...
    # Stakeがゲーム自動起動を廃止(リアルプレイクリック必須化)し、VPSのcamoufoxは
    # Cloudflare検問も通過できないため、ブラウザ経由のdga WSは新規セッションでは
    # 確立不能。dga lobby WSはcasinoIdのみで購読できる公開フィードなので、
    # Pythonから直接TLS接続して同じ on_ws_frame() に流し込む。
    # BACOPY_DGA_DIRECT=0 で無効化可。bot(dual_line_pragmatic_bot.py)は同名メソッド
    # を自前定義しておりそちらが優先される(こちらは素のCollector用)。
    ...
    def run(self, duration: int | None = None, profile_dir: Path | None = None,
        ...
        with Camoufox(**launch_opts) as ctx:
            ...
            try:
                logger.info(f"Warming up via {HOME_URL}")
                page.goto(HOME_URL, wait_until="domcontentloaded", timeout=60000)
                page.wait_for_timeout(5000)
            except Exception as e:
                logger.warning(f"home warmup failed: {e}")
