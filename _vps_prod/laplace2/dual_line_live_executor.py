        ...
        args = {
            ...
            "maxScroll": int(
                os.getenv(
                    ...
                    "6" if intent == "preposition" else "18",
                )
                or ("6" if intent == "preposition" else "18")
            ),
    ...
    def _perform_switch(self, req: dict) -> None:
        ...
        if self._multi_lobby_mode and intent != "fallback_join":
            ...
            if not ok and intent == "preposition" and target:
                cooldown = float(os.getenv("BACOPY_PREPOSITION_FOCUS_FAIL_COOLDOWN_SEC", "45") or 45)
...
(args) => {
  ...
  function guardGame(tile, src) {
    ...
    // In multi-baccarat, WS betsopen can advance before the compact tile text
    // redraws its ID. Allow clicking only for this known stale display case;
    // Python still requires lpbet confirmation for expectedGameId afterwards.
    if (staleTileGameIdOk && currentGameId === staleTileGameIdOk) return null;
    return {
      ok: false,
...
"""
    ...
    def _click_place_bet(
        ...
        def wait_for_matching_tile(click_index: int) -> bool:
            wait_sec = float(os.getenv("BACOPY_TILE_GAME_ID_WAIT_SEC", "2.5") or 2.5)
        ...
        def click_side_once(click_index: int) -> bool:
            ...
            if is_tile_game_id_mismatch() and wait_for_matching_tile(click_index):
                return True
