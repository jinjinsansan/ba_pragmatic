...
def predict_bead(row_cells):
    if len(row_cells) < 3 or 'T' in row_cells: return '', ''
    pb = row_cells
    shape = []
    ...
    p_cnt = pb.count('P'); b_cnt = pb.count('B')
    if p_cnt >= 3 and not (b_cnt >= 3): return 'P', 'pline'
    if b_cnt >= 3 and not (p_cnt >= 3): return 'B', 'bline'
    return '', ''
