...
V4_PATTERNS = frozenset({
    "sansan|telecho|P", "bline|telecho|B",
    "niconico|nikoichi|B", "niconico|dragon|B",
...
def _send_telegram(text: str) -> bool:
    ...
    if not token or not chat_id:
        ...
        return False
    # ★ミラー配信(2026-07-01): DUAL_LINE_EXTRA_CHAT_IDS(カンマ区切り)にも同じ本文を送る(best-effort・既存チャンネルに影響なし)
    _extra = os.getenv("DUAL_LINE_EXTRA_CHAT_IDS", "").strip()
    if _extra:
        try:
            import requests as _rq
            for _cid in [c.strip() for c in _extra.split(",") if c.strip()]:
                try:
                    _rq.post(f"https://api.telegram.org/bot{token}/sendMessage", json={"chat_id": _cid, "text": text}, timeout=5)
                except Exception:
                    pass
        except Exception:
            pass
...
def _send_telegram_result(text: str) -> bool:
    """結果専用チャンネル(NOW決済結果のみ: W/L/T・勝率・累積)への送信。
    DUAL_LINE_RESULT_CHAT_ID 未設定なら何もしない(既存チャンネルへの影響ゼロ)。"""
    # 宛先はカンマ区切りで複数指定できる(2026-08-03: 追従監視3を追加)
    chat_ids = [c.strip() for c in os.getenv("DUAL_LINE_RESULT_CHAT_ID", "").split(",") if c.strip()]
    if not chat_ids:
        return False
    token = (
        os.getenv("DUAL_LINE_BOT_TOKEN", "").strip()
    ...
    for _cid in chat_ids:
        try:
            ...
            if not response.ok:
                logger.warning(
                    f"[TELEGRAM-RESULT] send failed chat={_cid} status={response.status_code} "
                    f"response={response.text[:160]!r}"
                )
                continue
            ok_any = True
        except Exception as e:
            logger.warning(f"[TELEGRAM-RESULT] send failed chat={_cid}: {e}")
    return ok_any
...
class DualLinePragmaticBot(cp.Collector):
    ...
    def _on_ws(self, ws):
        ...
        # dga直結フィード稼働中はブラウザ側dga WSを二重取り込みしない
        if getattr(self, "_dga_direct_on", False) and cp.PRAGMATIC_WS_PATTERN in ws.url:
            logger.info("[DGA-DIRECT] browser dga WS ignored (direct feed active)")
            return
        super()._on_ws(ws)
        url = ws.url
    ...
    # Stakeがゲーム自動起動を廃止(リアルプレイクリック必須+難読化ドメイン化)し、
    # さらにVPSのcamoufoxはCloudflare検問(Just a moment...)を通過できないため、
    # ブラウザ経由のdga WSは新規セッションでは確立不能になった。
    # dga lobby WS は casinoId のみで購読できる公開フィード(Stakeログイン/Cookie不要・
    # 受け子executorの直結実装と同プロトコル)なので、Pythonから直接TLS接続して
    # 同じ on_ws_frame() に流し込む。BACOPY_DGA_DIRECT=0 で無効化可。
    ...
    def _on_shoe_change(self, table_id: str, buf):
        ...
        if table_id in self.pending:
            ...
            pass  # シュー変化はTelegram抑止（チャンネルは予告/結果のみ）
    ...
    def _v4_track(self, table_id, buf, new_hand, history_hands=None):
        ...
        if pend:
            ...
            if outcome == "T":
                self._v4["ties"] += 1; res = "TIE"; icon = "🔵"; pnl = 0.0
            elif outcome == side:
            ...
            try:
                ...
                pass
            if tg_on:
                _send_telegram(_v4_res_text)
            # 結果専用チャンネル(設定時のみ): フレッシュ集計(NOW+追従合算)で配信し追従をarm
            try:
                self._follow_sim_now_result(
                    table_id, buf.table_name or table_id, "v4", pk, side, res, outcome, new_hand)
            except Exception as _fse:
                logger.debug("[follow-sim] v4 now-result error: %s" % _fse)
        ...
        if tg_on:
            ...
            logger.info("[V4-SIGNAL-TELEGRAM] firing #%d" % self._v4["signals"])
            _send_telegram(
                "🔵【10パターン】SIGNAL #%d\nTable: %s\nPattern: %s\n"
                "Predict: %s (%s)\nW/L/T: %d/%d/%d (%.2f%%) | 解決: %d/%d" % (
                    self._v4["signals"], buf.table_name or table_id, pk, side, side_name,
                    self._v4["wins"], self._v4["losses"], self._v4["ties"], wr,
                    self._v4["resolved"], self._v4["signals"]))

    def _publish_v4_decision(self, table_id, buf, side, pattern_key):
    ...
    def _settle_v4_decision(self, pend, outcome, new_hand):
    ...
    def _load_v4_state(self):
        try:
            ...
            _changed = sorted(s.get("patterns") or []) != sorted(V4_PATTERNS)
            ...
            _pp = s.get("per_pattern", {}) or {}
            if _changed:
                # 2026-06-23: パターン変更でも通算(signals/W/L/T)は維持。外したパターンの
                # 内訳だけ落とす(寄与は通算に残り新パターンで徐々に薄まる)。旧=全リセットで
                # テレグラム通算が0に戻る不具合だった。
                _pp = {k: v for k, v in _pp.items() if k in V4_PATTERNS}
                logger.info("[v4] pattern set changed -> keep cumulative, prune per-pattern")
            self._v4_per_pattern = _pp
    ...
    # ── 仮想追従(follow)シミュレータ ──────────────────────────────
    # 受け子と同じ追従ルールをVPS側で仮想実行し、結果を結果専用チャンネル
    # (DUAL_LINE_RESULT_CHAT_ID)へ配信する。実BETなし・統計のみ。
    # ルール: NOW/追従手が TIE=同方向再BET / WIN=大路telecho(逆張り)・dragon(順張り)
    # のみ次手へ / LOSE=チェーン終了。フラット1単位(B勝ちはCOMMISSION_BANKER)。
    _FOLLOW_SIM_STATE_PATH = "/opt/laplace2/dual_line_follow_sim.json"

    def _follow_sim_state(self):
        # 目的=「追従は得か?損か?」を測る。集計は追従対象パターン(大路telecho/dragon)限定。
        # スキーマ: w/l/t/npnl=対象パターンのNOW成績, fw/fl/ft/fpnl=追従手の成績,
        # chains=追従チェーン数, best=最高連勝(NOW込み)。チャンネル開設時点から0スタートの
        # 独自集計で、botのlifetime累計(数千シグナル)とは完全に別物。
        if not hasattr(self, "_fsim"):
            self._fsim = {}
            _empty = {"w": 0, "l": 0, "t": 0, "npnl": 0.0,
                      "fw": 0, "fl": 0, "ft": 0, "fpnl": 0.0,
                      "chains": 0, "best": 0}
            self._fstats = {"v3": dict(_empty), "v4": dict(_empty)}
            try:
                import json as _json
                with open(self._FOLLOW_SIM_STATE_PATH, "r", encoding="utf-8") as f:
                    saved = _json.load(f)
                for k in ("v3", "v4"):
                    if isinstance(saved.get(k), dict):
                        for kk in _empty:
                            if kk in saved[k]:
                                self._fstats[k][kk] = saved[k][kk]
            except Exception:
                pass
        return self._fsim, self._fstats

    def _follow_sim_save(self):
        try:
            import json as _json
            with open(self._FOLLOW_SIM_STATE_PATH, "w", encoding="utf-8") as f:
                _json.dump(self._fstats, f)
        except Exception:
            pass

    @staticmethod
    def _follow_sim_kind(pattern_key):
        try:
            big = str(pattern_key or "").split("|")[1].strip().lower()
        except Exception:
            return ""
        return big if big in ("telecho", "dragon") else ""

    @staticmethod
    def _follow_sim_gid(new_hand):
        return str((new_hand or {}).get("gameId") or (new_hand or {}).get("game_id") or "")

    @staticmethod
    def _follow_sim_stats_line(st):
        """「追従は得か?損か?」ブロック(追従対象パターン限定・開設から累計)。
        追従のみのpnlが+なら追従は得、-なら損(NOWに上乗せされる限界BETの収支)。"""
        nw, nl, nt = st["w"], st["l"], st["t"]
        fw, fl, ft = st["fw"], st["fl"], st["ft"]
        n_n = nw + nl
        n_f = fw + fl
        wr_n = (nw / n_n * 100) if n_n else 0.0
        wr_f = (fw / n_f * 100) if n_f else 0.0
        tw, tl = nw + fw, nl + fl
        n_t = tw + tl
        wr_t = (tw / n_t * 100) if n_t else 0.0
        return (
            "── 追従は得か?損か?(対象パターン限定) ──\n"
            "追従のみ: %dW/%dL/%dT (%.2f%%) pnl %+.1f ←ここが+なら得\n"
            "NOW単独: %dW/%dL/%dT (%.2f%%) pnl %+.1f\n"
            "追従込み: %dW/%dL (%.2f%%) pnl %+.1f\n"
            "最高連勝: %d | chains: %d" % (
                fw, fl, ft, wr_f, st["fpnl"],
                nw, nl, nt, wr_n, st["npnl"],
                tw, tl, wr_t, st["npnl"] + st["fpnl"],
                st.get("best", 0), st.get("chains", 0)))

    def _follow_sim_now_result(self, table_id, table_name, origin, pattern_key, side,
                               result, outcome, new_hand):
        """NOW決済時: 追従対象パターンのみ集計・配信し、追従をarmする。
        追従が発生し得ないパターン(大路がtelecho/dragon以外)は集計を濁すため
        集計外(コンパクト配信のみ)。"""
        if not os.getenv("DUAL_LINE_RESULT_CHAT_ID", "").strip():
            return
        fsim, fstats = self._follow_sim_state()
        st = fstats.get(origin) or fstats["v3"]
        kind = self._follow_sim_kind(pattern_key)
        gid = self._follow_sim_gid(new_hand)
        tag = "🟢【6パターン】" if origin == "v3" else "🔵【10パターン】"
        icon = "✅" if result == "WIN" else ("❌" if result == "LOSE" else "🔵")
        if not kind:
            # 追従対象外パターン: 集計せず結果だけコンパクトに流す
            _send_telegram_result(
                "%s%s %s %s\nPattern: %s (追従対象外・集計外)\nPred: %s → Got: %s" % (
                    tag, icon, result, table_name, pattern_key, side, outcome) + _recent_wl_show(origin))
            fsim.pop(table_id, None)
            return
        if result == "WIN":
            st["w"] += 1
            st["npnl"] += COMMISSION_BANKER if side == "B" else 1.0
            if st.get("best", 0) < 1:
                st["best"] = 1
        elif result == "LOSE":
            st["l"] += 1
        ...
        if result == "TIE":
            ...
            nxt_label = "\n→ 追従開始(%s)" % ("逆張り" if kind == "telecho" else "順張り")
        _send_telegram_result(
            "%s%s %s %s\nPattern: %s\nPred: %s → Got: %s\n%s%s" % (
                tag, icon, result, table_name, pattern_key, side, outcome,
                self._follow_sim_stats_line(st), nxt_label) + _recent_wl_show(origin))
        # ── 追従arm ──
        if result == "TIE":
            fsim[table_id] = {"origin": origin, "side": side, "kind": kind, "chain": 0,
                              "streak": 0, "pattern_key": pattern_key,
                              "table_name": table_name, "armed_gid": gid}
        elif result == "WIN":
            nxt = ("P" if side == "B" else "B") if kind == "telecho" else side
            st["chains"] += 1
            fsim[table_id] = {"origin": origin, "side": nxt, "kind": kind, "chain": 1,
                              "streak": 1, "pattern_key": pattern_key,
                              "table_name": table_name, "armed_gid": gid}
        else:
            fsim.pop(table_id, None)
        self._follow_sim_save()

    def _follow_sim_on_hand(self, table_id, buf, new_hand, outcome):
        """全卓の各確定ハンドで呼ぶ。armed済みの仮想追従手を決済し結果を配信。"""
        if not os.getenv("DUAL_LINE_RESULT_CHAT_ID", "").strip():
            return
        fsim, fstats = self._follow_sim_state()
        sim = fsim.get(table_id)
        if not sim:
            return
        gid = self._follow_sim_gid(new_hand)
        if gid and gid == str(sim.get("armed_gid") or ""):
            return  # seed元と同じハンドでは決済しない
        origin = str(sim.get("origin") or "v3")
        side = str(sim.get("side") or "P")
        kind = str(sim.get("kind") or "")
        chain = int(sim.get("chain") or 0)
        streak = int(sim.get("streak") or 0)
        st = fstats.get(origin) or fstats["v3"]
        chain_label = "再BET" if chain == 0 else ("追従%d手目" % chain)
        if outcome == "T":
            st["ft"] += 1; res = "TIE"; icon = "🔵"
            sim["armed_gid"] = gid  # 同方向で再BET(再arm)
            head = "%s=TIE → 同方向で再BET(連勝%d継続)" % (chain_label, streak)
        elif outcome == side:
            st["fw"] += 1; res = "WIN"; icon = "✅"
            st["fpnl"] += COMMISSION_BANKER if side == "B" else 1.0
            streak += 1
            if streak > st.get("best", 0):
                st["best"] = streak
            head = "🔥%d連勝中 (%s)" % (streak, chain_label)
        else:
            st["fl"] += 1; res = "LOSE"; icon = "❌"
            st["fpnl"] -= 1.0
            head = "チェーン終了 — %d連勝でストップ (%s)" % (streak, chain_label)
        tag = "🟢6パターン" if origin == "v3" else "🔵10パターン"
        _send_telegram_result(
            "🟡【追従】%s %s %s\n%s\nOrigin: %s %s\nBet: %s → Got: %s\n%s" % (
                icon, res, sim.get("table_name") or table_id, head,
                tag, sim.get("pattern_key") or "-", side, outcome,
                self._follow_sim_stats_line(st)) + _recent_wl_show(origin))
        if res == "WIN":
            if kind:
                nxt = ("P" if side == "B" else "B") if kind == "telecho" else side
                fsim[table_id] = {"origin": origin, "side": nxt, "kind": kind,
                                  "chain": chain + 1, "streak": streak,
                                  "pattern_key": sim.get("pattern_key"),
                                  "table_name": sim.get("table_name"), "armed_gid": gid}
            else:
                fsim.pop(table_id, None)  # 非追従パターンの再BET勝ち=終了
        elif res == "LOSE":
            fsim.pop(table_id, None)
        self._follow_sim_save()
    ...
    def _on_new_hand(self, table_id: str, buf, new_hand: dict, history_hands: list[dict] | None = None):
        ...
        try:
            self._follow_sim_on_hand(table_id, buf, new_hand, outcome_char)
        except Exception as _fse:
            logger.debug(f"[follow-sim] on_hand error: {_fse}")
        ...
        if self.bet_executor.is_live:
            ...
            has_pending = False
            try:
                has_pending = bool(self.bet_executor.has_pending_bet)
            except Exception:
                has_pending = False
            if has_pending:
    ...
    def _resolve_prediction(
        ...
        _v3_n_nt = self.wins + self.losses
        _v3_wr = self.wins / _v3_n_nt * 100 if _v3_n_nt else 0
        _v3_icon = "✅" if result == "WIN" else ("🔵" if result == "TIE" else "❌")
        _v3_res_text = (
            f"🟢【6パターン】{_v3_icon} {result} {buf.table_name or table_id}\n"
            f"Pattern: {_fmt_pattern(pkey)}\n"
            f"Pred: {side} → Got: {outcome}\n"
            f"W/L/T: {self.wins}/{self.losses}/{self.ties} ({_v3_wr:.2f}%)\n"
        ...
        if self.notify_resolution and (result != "TIE" or self.notify_tie):
            _send_telegram(_v3_res_text)
        # 結果専用チャンネル(設定時のみ): フレッシュ集計(NOW+追従合算)で配信し追従をarm
        try:
            self._follow_sim_now_result(
                table_id, buf.table_name or table_id, "v3", pkey, side, result, outcome, new_hand)
        except Exception as _fse:
            logger.debug("[follow-sim] v3 now-result error: %s" % _fse)
    ...
    def _notify_signal(
        ...
        wr = self.wins / n_nt * 100 if n_nt else 0
        _send_telegram(
            f"🟢【6パターン】SIGNAL #{self.total_signals}\n"
            f"Table: {buf.table_name or table_id}\n"
            f"Pattern: {_fmt_pattern(pattern_key)}\n"
            f"Predict: {side} ({side_name}) | Bet: ${bet_amt:.2f}\n"
            f"W/L/T: {self.wins}/{self.losses}/{self.ties} ({wr:.2f}%) | 解決: {self.total_resolved}/{self.total_signals}"
        )
    ...
    def _check_decision_results(self) -> None:
        ...
        for d in rows:
            ...
            try:
                ...
                if bet_id_for_consume and hasattr(self.bet_executor, "consume_sent_bet"):
    ...
    def run(
        ...
        # dga直結フィード (Stake/Cloudflare完全バイパス)。2026-08-03のStake仕様変更
        # (自動起動廃止)によりブラウザ経由フィードが新規セッションで死ぬため必須。
        ...
        try:
            ...
            try:
                _home = getattr(cp, "HOME_URL", "https://stake.com/ja/casino/home")
                logger.info(f"[BOT] warming up via {_home}")
                bet_page.goto(_home, wait_until="domcontentloaded", timeout=60000)
                bet_page.wait_for_timeout(5000)
            except Exception as e:
                logger.warning(f"[BOT] home warmup failed: {e}")
