CREATE TABLE IF NOT EXISTS ledger_actual_balances (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  investor_id UUID NOT NULL REFERENCES ledger_investors(id) ON DELETE CASCADE,
  snapshot_date DATE NOT NULL,
  account1_actual NUMERIC(14,2),
  account2_actual NUMERIC(14,2),
  charge_actual NUMERIC(14,2),
  notes TEXT,
...
CREATE INDEX IF NOT EXISTS idx_lab_investor_date
  ON ledger_actual_balances(investor_id, snapshot_date DESC);
...
DROP TRIGGER IF EXISTS trg_lab_updated_at ON ledger_actual_balances;
CREATE TRIGGER trg_lab_updated_at
BEFORE UPDATE ON ledger_actual_balances
FOR EACH ROW EXECUTE FUNCTION update_lab_updated_at();

ALTER TABLE ledger_actual_balances ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS lab_admin_all ON ledger_actual_balances;
CREATE POLICY lab_admin_all ON ledger_actual_balances
  FOR ALL USING (
    EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.is_admin = TRUE)
  );

DROP VIEW IF EXISTS ledger_investor_summary;

CREATE VIEW ledger_investor_summary AS
WITH a1 AS (
  SELECT investor_id,
         ...
         SUM(investor_recharge) AS investor_recharge_total
    FROM ledger_account1_daily GROUP BY investor_id
),
a2 AS (
  SELECT investor_id,
    ...
    FROM ledger_account2_daily GROUP BY investor_id
),
exp AS (
  SELECT investor_id,
    ...
    FROM ledger_expense_withdrawals GROUP BY investor_id
),
breakdown AS (
  SELECT investor_id, SUM(amount) AS company_breakdown_total
    FROM ledger_company_expense_breakdown GROUP BY investor_id
),
latest_snap AS (
  SELECT DISTINCT ON (investor_id)
         investor_id, snapshot_date,
         account1_actual, account2_actual, charge_actual
    FROM ledger_actual_balances
   ORDER BY investor_id, snapshot_date DESC
)
SELECT
  i.id AS investor_id, i.name AS investor_name, i.total_investment,
  i.account1_amount, i.account2_amount, i.initial_charge_display,
  ...
  i.initial_charge_display - COALESCE(a1.account1_80pct_total, 0) + COALESCE(a1.investor_recharge_total, 0) AS displayed_charge_balance,
  ...
  (COALESCE(a1.account1_80pct_total, 0) + COALESCE(a2.account2_total_profit, 0)) - COALESCE(exp.expense_from_account2, 0) AS operator_remaining_profit,
  ...
  COALESCE(a1.investor_recharge_total, 0) AS investor_recharge_total,
  COALESCE(a1.account1_total_profit, 0) - COALESCE(a1.investor_recharge_total, 0) AS investor_net_received,
  (COALESCE(a1.account1_total_profit, 0) - COALESCE(a1.investor_recharge_total, 0)) - COALESCE(a1.investor_received_total, 0) AS investor_overpaid,
  ls.snapshot_date AS actual_snapshot_date,
  ls.account1_actual,
  ls.account2_actual,
  ls.charge_actual,
  COALESCE(ls.account1_actual, 0) + COALESCE(ls.account2_actual, 0) + COALESCE(ls.charge_actual, 0) AS physical_total,
  i.account1_amount + i.account2_amount + COALESCE(rf.initial_amount, 0)
    + COALESCE(a2.account2_total_profit, 0)
    + COALESCE(a1.investor_recharge_total, 0)
    - COALESCE(exp.expense_total, 0) AS expected_physical_total,
  (COALESCE(ls.account1_actual, 0) + COALESCE(ls.account2_actual, 0) + COALESCE(ls.charge_actual, 0))
    - (i.account1_amount + i.account2_amount + COALESCE(rf.initial_amount, 0)
       + COALESCE(a2.account2_total_profit, 0)
       + COALESCE(a1.investor_recharge_total, 0)
       - COALESCE(exp.expense_total, 0)) AS physical_diff
...
LEFT JOIN latest_snap ls ON ls.investor_id = i.id;

INSERT INTO ledger_actual_balances (investor_id, snapshot_date, account1_actual, account2_actual, charge_actual, notes)
VALUES ('00000000-0000-0000-0000-000000000001'::uuid, '2026-05-09'::date, 50000.00, 57828.00, 4000.00, '5/9 報告 (Hさん)')
ON CONFLICT (investor_id, snapshot_date) DO UPDATE SET
  account1_actual = EXCLUDED.account1_actual,
  account2_actual = EXCLUDED.account2_actual,
  charge_actual = EXCLUDED.charge_actual,
  notes = EXCLUDED.notes;

SELECT
  investor_name,
  account1_actual, account2_actual, charge_actual, physical_total,
  expected_physical_total, physical_diff,
  account2_balance,
  displayed_charge_balance,
  account1_amount AS account1_expected
FROM ledger_investor_summary WHERE investor_name = 'H';
